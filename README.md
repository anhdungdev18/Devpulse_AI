...
- Deploy thư mục `frontend/` lên static hosting như Vercel, Netlify hoặc Nginx.
- Sửa `frontend/config.js` để trỏ `API_BASE_URL` về URL backend của bạn.
...
1. **Multi-provider models** — Swap `RelevanceAgent` to use another provider such as Anthropic Claude by updating the model config. The `agno` framework supports multiple providers.
...
fastapi           # Web API for custom dashboard
uvicorn[standard] # ASGI server
```

No additional provider SDKs are required for the default setup. If you want multi-provider support later, install the corresponding SDK separately.
