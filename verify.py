...
def verify_relevance_agent(signals: List[Dict]):
    ...
    assert all("factors" in s["relevance"] for s in scored), "Missing relevance factors"
    assert all(
...
def verify_risk_agent(signals: List[Dict]):
    ...
    assert all("risk_score" in s["risk"] for s in assessed), "Missing risk_score"
    assert all("factors" in s["risk"] for s in assessed), "Missing risk factors"
